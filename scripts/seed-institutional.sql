...
INSERT INTO "Company" (
  ...
  'seed-company-1','Companhia Teste','companhia-teste','Sao Paulo','SP','ACTIVE', DATETIME('now'), DATETIME('now')
);
...
(
  ...
  NULL,'institucional@test.local',NULL, DATETIME('now'), NULL, NULL, DATETIME('now'), DATETIME('now')
),
(
  'seed-int-webhook-1','Webhook SP','CITY','WEBHOOK','ACTIVE', NULL,'Sao Paulo','SP','https://example.com/hook','POST', NULL,
  NULL, NULL, NULL, DATETIME('now'), NULL, NULL, DATETIME('now'), DATETIME('now')
);
...
('seed-email-1','institucional@test.local','Relatorio','Payload','SENT',NULL,NULL,'seed-int-email-1',NULL,NULL,NULL,
  DATETIME('now','-2 hours'), DATETIME('now','-1 hour', '-50 minutes')),
('seed-email-2','institucional@test.local','Relatorio','Payload','SENT',NULL,NULL,'seed-int-email-1',NULL,NULL,NULL,
  DATETIME('now','-3 hours'), DATETIME('now','-2 hours','-50 minutes')),
...
('seed-hook-1','https://example.com/hook','POST',NULL,'ok','SENT',NULL,NULL,'seed-int-webhook-1', DATETIME('now','-90 minutes'), DATETIME('now','-88 minutes')),
('seed-hook-2','https://example.com/hook','POST',NULL,'ok','FAILED','timeout',NULL,'seed-int-webhook-1', DATETIME('now','-40 minutes'), NULL),
('seed-hook-3','https://example.com/hook','POST',NULL,'ok','PENDING',NULL,NULL,'seed-int-webhook-1', DATETIME('now','-15 minutes'), NULL);
