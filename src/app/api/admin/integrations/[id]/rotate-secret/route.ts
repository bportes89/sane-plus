...
import { encryptIntegrationSecret, generateToken } from "@/lib/integrations";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rl = rateLimit({ key: `admin:integrations:rotate-secret:${user.id}:${ip}`, limit: 20, windowMs: 60_000 });
  if (!rl.ok) {
  ...
  if (integration.kind !== IntegrationKind.WEBHOOK) return NextResponse.json({ error: "invalid_kind" }, { status: 400 });
  if (integration.status !== IntegrationStatus.ACTIVE) return NextResponse.json({ error: "inactive" }, { status: 400 });

  const secret = generateToken(32);
  await prisma.integration.update({ where: { id }, data: { signingSecretEnc: encryptIntegrationSecret(secret) } });
  return NextResponse.json({ ok: true, signingSecret: secret });
}
